...
# getY, b2g, g2b, getP1, device, bitLen, blen

class DISC:
    def __init__(self, key: int, random_seed: int, payloadBits: str = "", rLambda: float = 4.0, t: float=1.0, contextChecking: bool = False, windowSz: int = 8):
    ...
    def decode(self, tokenIds: List[int], msgLenHyp: int) -> Dict:
        ...
        def _pvalue_for_hyp(nStar, mPrime, all_token_ids):
            if not (0 <= mPrime < msgSpaceHyp): return 1.0, 0.0
            deltaMPrime = mPrime * (1.0 / msgSpaceHyp)
        ...
        def _process_nstar(nStar):
            ...
            for mPrimeCoarse in coarseCandidates:
                pVal, _ = _pvalue_for_hyp(nStar, mPrimeCoarse, tokenIds); tested += 1
            ...
            for mPrimeFine in range(fineStart, fineEnd):
                ...
                pVal, _ = _pvalue_for_hyp(nStar, mPrimeFine, tokenIds); tested += 1
                if pVal < bestPValHere: bestPValHere, bestMHere = pVal, mPrimeFine
            return bestPValHere, nStar, bestMHere, tested
        
        results = [_process_nstar(n) for n in tqdm(range(0, totalBits + 1, bitLen), desc="Sweeping n* for DISC")]
