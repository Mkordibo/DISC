...
class Christ:
    def __init__(self, key: int, salt: int, rLambda: float, random_seed: int, t: float = 1.0, payload: Optional[str] = None, scoreThreshold: Optional[float] = None, isGeneral: bool = True):
        ...
        self.Wb = 2
        self.payload = self.payload.zfill(math.ceil(len(payload)/self.Wb)*self.Wb)
        self.payload_ints = [int(self.payload[i:i+self.Wb],2)for i in range(0,len(self.payload),self.Wb)]
        self.W = 400*15
    ...
    def __call__(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        Ys = getYs(self.seed_bytes, self.seed_bytes, None, bitLen) if self.inH else getYs(self.salt_bytes, self.key_bytes, [self.payload_ints[(self.bitIdx-len(self.r))//self.W], self.r, self.tkIdx], bitLen)
        newTokenId=0
        for bitIdx in range(bitLen):
            ...
            if self.isGeneral and self.h>=self.rLambda: # sometimes invalidate self.inH at bit boundary if isGeneral status, update Ys
                self.inH = False
                Ys = getYs(self.salt_bytes, self.key_bytes, [self.payload_ints[0], self.r, self.tkIdx], bitLen)
            self.bitIdx+=1
            if not self.inH and self.bitIdx-len(self.r)%self.W==0:
                Ys = getYs(self.salt_bytes, self.key_bytes, [self.payload_ints[(self.bitIdx-len(self.r))//self.W], self.r, self.tkIdx], bitLen)
